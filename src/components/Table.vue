<template>
  ...
  <div class="wrapper" @scroll="mouseUp">
    ...
    <div class="loading" v-if="arr.length !== total">
      loading...
    </div>
...
export default {
  ...
  setup(props) {
    ...
    async function getTableData() {
      const res = await getSosList("/sos/SOSList?", "get", props.query);
      props.changeCurPage();
      if (res.code === 0) {
        return res.data;
      }
    ...
    /*
      ...
      如果此时不触发滚动事件则不会被监听到，导致tableData不会重新刷新
    ...
    let showTr = computed(() => {
      return [
        ...tableData.arr.slice(
          ...
          (startNum.value >= 0 ? startNum.value : 0) + 60
        ),
      ];
    });
